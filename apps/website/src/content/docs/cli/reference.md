...
`~/.config/open-ota/config.json`, written `0600`. It prompts for an API token or
an email and password, and checks the credential against the server before
saving it.
...
`--provider <name>` (`supabase`, `cloudflare`, `docker` — prints or runs backend
provisioning) · `--project <id>` · `-c, --channel <name>` (default `production`)
· `--scheme <scheme>` (deep link scheme for preview links) · `--dry-run` ·
`-y, --yes`

## fingerprint

`ota fingerprint --check` compares the committed `fingerprint.json` against the
project and fails on drift, which is what you want in CI. Without `--check` it
writes the file. Commit it: it is the contract between a bundle and the
binaries allowed to run it.
...
`-m, --message <text>` · `--bundle-dir <dir>` (publish a prebuilt export instead
of running `expo export`) · `--project <id>` · `--dry-run` (build and hash
without publishing) · `--json`

## releases, release

`ota releases -c production --status active` lists releases newest first.
`ota release v42 --platform android` shows one, by id or by label. Labels are
per channel and platform, so pass `--platform` when the same label exists on
both.
...
`release <labelOrId>`: `-c, --channel <name>` · `-p, --platform <platform>` ·
`--project <id>` · `--json`

## promote, rollout

`ota promote v42 production --rollout 25` copies a release into another channel
as a new release with its own id and label, reusing the same bundle.
`ota rollout v42 50` changes the percentage; lowering it warns that it stops
new devices only, because devices already on the release keep it.
...
keep it. `ota resume v43` offers it again. `ota disable v43` pulls it, and
devices converge to the previous release or the embedded bundle.
`ota rollback -c production` does the same to the newest active release on a
channel, per platform, and reports where devices will land. Both `disable` and
`rollback` confirm first.
...
`ota preview v42 --ttl 60` prints a QR code that opens exactly that release on
a device with the app installed. The app must already have a matching
fingerprint, and the preview stays pinned until `exitPreview()`.
...
fingerprint drift and a conflicting `expo-updates` install. It exits 1 if any
check fails.
...
`ota mcp` runs the MCP server over stdio. See
...
environment or the machine file, never from the committed project file.

The project root is the nearest ancestor directory containing
`ota.config.json`, otherwise the nearest containing `package.json`.
